            ...
            if let tabConfiguration {
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 18) {
                        // A zero-width title is the opt-in marker for a
                        // presentation-only extension tab. It keeps the tab
                        // selectable in Atoll's global strip while allowing
                        // the extension's own content to use the full panel
                        // without a duplicated native header/icon.
                        if tabConfiguration.title != "\u{200B}" {
