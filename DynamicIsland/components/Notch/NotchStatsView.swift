...
struct NotchStatsView: View {
    ...
    var statsGridLayout: some View {
        ...
        if graphCount <= 3 {
            ...
            // A single 2×2 grid makes every card share the exact same column width.
            LazyVGrid(
                columns: Array(repeating: GridItem(.flexible(minimum: 0), spacing: 8), count: 2),
                spacing: statsGridSpacingHeight
            ) {
                ForEach(0..<graphCount, id: \.self) { index in
                    graphViewForIndex(index)
                }
